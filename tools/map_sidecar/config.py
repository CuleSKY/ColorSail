...
def load_settings() -> Settings:
    ...
    project_root = _get_env("PROJECT_ROOT", "/opt/example-app/NERV_CS2ZE")
    static_dir_name = _get_env("STATIC_DIR_NAME", "static")
    log_dir = _get_env("MAP_SIDECAR_LOG_DIR", os.path.join(project_root, "logs"), allow_empty=True)
    if log_dir == "":
        log_dir = None
    ...
    return Settings(
        ...
        main_servers_json_url=_get_env("MAIN_SERVERS_JSON_URL", "https://example.com/servers.json"),
...
def load_cn_fetcher_settings() -> CnFetcherSettings:
    ...
    return CnFetcherSettings(
        project_root=project_root,
        log_dir=log_dir,
        exg_maplist_url=_get_env("EXG_MAPLIST_URL", "https://example.com/serverlist/cs2maplist"),
