...
EXG_URL = os.environ.get("EXG_MAPLIST_URL", "https://example.com/serverlist/cs2maplist").strip()
...
def _parse_workshop(value: str) -> tuple[Optional[int], Optional[str]]:
    if not value:
        return None, None
    text = value.strip()
    url_match = re.search(r"https?://[^\s/]+/sharedfiles/filedetails/\?id=(\d+)", text)
    if url_match:
    ...
    if id_match:
        ...
        return workshop_id, f"{WORKSHOP_URL_PREFIX}{workshop_id}"
    return None, None
