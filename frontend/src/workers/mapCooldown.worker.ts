import { getExgStatusState, normalizeZh, stripBracketSegments } from '../mapSearchCore.js';

let cachedMapIndex: any = null
...
const buildRows = (payload: any, buildId?: number) => {
  ...
  const maybePostProgress = (doneNow: number, totalNow: number) => {
    const now = performance.now()
    if (
      doneNow === totalNow ||
      (doneNow - lastProgressDone) >= 2000 ||
      (now - lastProgressTs) >= 100
    ) {
      lastProgressDone = doneNow
      lastProgressTs = now
      self.postMessage({ type: 'PROGRESS', payload: { done: doneNow, total: totalNow, buildId } })
    }
  }
  ...
  const resolveMapLine2 = (key: string, entry: any) => {
    ...
    if (locale === 'zh-TW') {
      ...
      const mapTw = stripBracketSegments(mapTwRaw);
...
self.onmessage = (event: MessageEvent) => {
  ...
  // 兼容旧/新消息名
  if (data.type === 'SETINDEX' || data.type === 'SET_INDEX') {
    cachedMapIndex = data.payload?.mapIndex ?? null;
    return;
  }
  ...
  try {
    ...
    const mapIndex = payload.mapIndex ?? cachedMapIndex;
    if (!mapIndex) throw new Error('mapIndex not set');

    const { rows, coolingRows } = buildRows({ ...payload, mapIndex }, buildId);

    const meta = {
      totalRows: rows.length,
      coolingRows: coolingRows.length,
      rowHeight: payload?.rowHeight ?? null,
    };
