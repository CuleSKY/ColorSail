...
const buildRows = (payload: any) => {
  ...
  const resolveMapLine2 = (_key: string, entry: any) => {
  ...
  for (let i = 0; i < entries.length; i += chunkSize) {
    ...
    if (done < total) {
      self.postMessage({ type: 'BUILD_RESULT', payload: { progress: { done, total } } });
    }
  }
  ...
  return { rowsAll: rows, rowsCooling: coolingRows, progress: { done: rows.length, total: rows.length } };
};
...
self.onmessage = (event: MessageEvent) => {
  ...
  try {
    const { rowsAll, rowsCooling, progress } = buildRows(data.payload);
    self.postMessage({
      type: 'BUILD_RESULT',
      payload: {
        buildId,
