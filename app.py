...
ADMIN_HOSTNAME = os.environ.get('ADMIN_HOSTNAME', 'admin.example.com').strip().lower()
PUBLIC_HOSTNAMES = {
    host.strip().lower()
    for host in os.environ.get('PUBLIC_HOSTNAMES', 'example.com,www.example.com').split(',')
    if host.strip()
}
CANONICAL_HOST = os.environ.get('CANONICAL_HOST', 'example.com').strip().lower()
CANONICAL_SCHEME = os.environ.get('CANONICAL_SCHEME', 'https').strip().lower()
CANONICAL_ORIGIN = f"{CANONICAL_SCHEME}://{CANONICAL_HOST}"
EMBED_ALLOWED_ORIGIN = os.environ.get('EMBED_ALLOWED_ORIGIN', 'https://example.com').strip()

# EXG API 地址
EXG_API_URL = os.environ.get('EXG_API_URL', 'https://example.com/ServerList/CurrentStatus').strip()
...
STEAM_COMMUNITY_BASE_URL = os.environ.get('STEAM_COMMUNITY_BASE_URL', 'https://example.com').rstrip('/')
STEAM_OPENID_ENDPOINT = os.environ.get('STEAM_OPENID_ENDPOINT', f'{STEAM_COMMUNITY_BASE_URL}/openid/login').strip()
...
if cookie_domain_env:
    ...
    if cookie_domain_env.startswith('.'):
        allow_cross = os.environ.get('SESSION_COOKIE_DOMAIN_ALLOW_CROSS_SUBDOMAIN', '').lower() in ('1', 'true', 'yes')
        if not allow_cross:
            raise RuntimeError("SESSION_COOKIE_DOMAIN with leading dot is not allowed without explicit opt-in.")
...
def fetch_persona_name(steam_id):
    if not steam_id:
        return "Unknown"
    url = f"{STEAM_COMMUNITY_BASE_URL}/profiles/{steam_id}/?xml=1"
    try:
...
def build_embed_csp():
    return (
        ...
        "font-src 'self' data:; "
        "connect-src 'self'; "
        f"frame-ancestors {EMBED_ALLOWED_ORIGIN}; "
        "object-src 'none'; "
        "base-uri 'self'"
    )
...
def embed_servers():
    ...
    resp.headers['X-Frame-Options'] = f'ALLOW-FROM {EMBED_ALLOWED_ORIGIN}'
    return resp
...
def steam_callback():
    ...
    if expected_state is None:
        ...
        print("2. Host mismatch between login/callback domains")
...
def _beijing_day_str(ts):
    return (datetime.utcfromtimestamp(int(ts)) + timedelta(hours=8)).strftime('%Y-%m-%d')

def _day_range_strings(days):
...
@app.route('/api/stats/advanced', strict_slashes=False)
@app.route('/api/stats-advanced', strict_slashes=False)
def get_stats_advanced():
    ...
    bucket_seconds = 600
    expected_samples_per_day = int(24 * 60 / 10)
    day_keys_30 = _day_range_strings(days)
    day_keys_7 = day_keys_30[-7:]

    snapshot = get_config_snapshot()
    community_meta = list(snapshot.get('community_meta') or [])
    community_ids = [str(c.get('id')) for c in community_meta if c.get('id')]
    community_name_map = {
        str(c.get('id')): (c.get('short_name') or c.get('name') or str(c.get('id')))
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT timestamp, community, count FROM player_stats WHERE timestamp >= ? ORDER BY timestamp ASC",
            (cutoff_ts,)
        )
        rows = c.fetchall()
    except sqlite3.OperationalError as exc:
        err = str(exc).lower()
        if "no such table" in err and "player_stats" in err:
            current_app.logger.warning("player_stats table missing, returning empty advanced stats payload")
            rows = []
        else:
            raise
    finally:
    ...
    # Collapse to 10-minute bucket max for each (day, community, bucket).
    comm_bucket_max = {}
    for ts, cid, count in rows:
        if ts is None or cid is None:
            continue
        day = _beijing_day_str(ts)
        if day not in day_keys_30:
            continue
        key = (day, str(cid), int(ts) - (int(ts) % bucket_seconds))
        prev = comm_bucket_max.get(key)
        if prev is None or int(count or 0) > prev:
            comm_bucket_max[key] = int(count or 0)

    # Build per-day/per-community stats and per-day total bucket stats.
    day_comm_stats = {}
    day_total_buckets = {}
    seen_communities = set()
    for (day, cid, bucket), value in comm_bucket_max.items():
        seen_communities.add(cid)
        day_entry = day_comm_stats.setdefault(day, {})
        comm_entry = day_entry.setdefault(cid, {"pcu": 0, "sum": 0.0, "samples": 0})
        if value > comm_entry["pcu"]:
            comm_entry["pcu"] = value
        comm_entry["sum"] += float(value)
        comm_entry["samples"] += 1

        day_total = day_total_buckets.setdefault(day, {})
        day_total[bucket] = day_total.get(bucket, 0) + value

    # Ensure unknown historical communities are still listed.
    all_communities = list(community_ids)
    for cid in sorted(seen_communities):
        if cid not in all_communities:
            all_communities.append(cid)
            community_name_map[cid] = cid

    # Total daily stats.
    day_total_stats = {}
    for day in day_keys_30:
        bucket_map = day_total_buckets.get(day, {})
        values = list(bucket_map.values())
        samples = len(values)
        day_total_stats[day] = {
            "pcu": float(max(values) if values else 0.0),
            "acu": float(_safe_avg(values)),
            "samples": samples
        }

    # Community daily rows for 7d/30d (descending date for table readability).
    def build_daily_rows(day_keys):
        rows_out = []
        for day in reversed(day_keys):
            day_entry = day_comm_stats.get(day, {})
            for cid in all_communities:
                raw = day_entry.get(cid, {"pcu": 0, "sum": 0.0, "samples": 0})
                samples = int(raw.get("samples", 0))
                acu = float(raw.get("sum", 0.0) / samples) if samples > 0 else 0.0
                rows_out.append({
                    "date": day,
                    "community_id": cid,
                    "community_name": community_name_map.get(cid, cid),
                    "pcu": float(raw.get("pcu", 0.0)),
                    "acu": acu,
                    "samples": samples,
                    "expected_samples": expected_samples_per_day
                })
        return rows_out

    community_daily_rows_7d = build_daily_rows(day_keys_7)
    community_daily_rows_30d = build_daily_rows(day_keys_30)

    # Monthly summary.
    prev_week_days = day_keys_30[-14:-7] if len(day_keys_30) >= 14 else []
    monthly_summary = []
    for cid in all_communities:
        pcu_series = []
        acu_series = []
        for day in day_keys_30:
            raw = day_comm_stats.get(day, {}).get(cid, {"pcu": 0, "sum": 0.0, "samples": 0})
            samples = int(raw.get("samples", 0))
            pcu = float(raw.get("pcu", 0.0))
            acu = float(raw.get("sum", 0.0) / samples) if samples > 0 else 0.0
            pcu_series.append(pcu)
            acu_series.append(acu)

        recent_pcu = [pcu_series[day_keys_30.index(d)] for d in day_keys_7]
        recent_acu = [acu_series[day_keys_30.index(d)] for d in day_keys_7]
        prev_pcu = [pcu_series[day_keys_30.index(d)] for d in prev_week_days] if prev_week_days else []
        prev_acu = [acu_series[day_keys_30.index(d)] for d in prev_week_days] if prev_week_days else []

        recent_pcu_avg = _safe_avg(recent_pcu)
        recent_acu_avg = _safe_avg(recent_acu)
        prev_pcu_avg = _safe_avg(prev_pcu)
        prev_acu_avg = _safe_avg(prev_acu)

        pcu_trend_pct = 0.0 if prev_pcu_avg <= 0 else ((recent_pcu_avg - prev_pcu_avg) / prev_pcu_avg) * 100.0
        acu_trend_pct = 0.0 if prev_acu_avg <= 0 else ((recent_acu_avg - prev_acu_avg) / prev_acu_avg) * 100.0

        monthly_summary.append({
            "community_id": cid,
            "community_name": community_name_map.get(cid, cid),
            "avg_pcu_30d": _safe_avg(pcu_series),
            "avg_acu_30d": _safe_avg(acu_series),
            "pcu_peak_30d": float(max(pcu_series) if pcu_series else 0.0),
            "acu_peak_30d": float(max(acu_series) if acu_series else 0.0),
            "pcu_trend_pct": float(pcu_trend_pct),
            "acu_trend_pct": float(acu_trend_pct)
        })

    # Total charts for 7d / 30d.
    def build_total_chart(day_keys):
        return {
            "labels": day_keys,
            "datasets": [
                {
                    "label": "PCU",
                    "data": [round(day_total_stats[d]["pcu"], 2) for d in day_keys],
                    "borderColor": "#ff4d4f",
                ...
                {
                    "label": "ACU",
                    "data": [round(day_total_stats[d]["acu"], 2) for d in day_keys],
                    "borderColor": "#3b82f6",
    ...
    payload = {
        ...
        "community_daily_rows_7d": community_daily_rows_7d,
        "community_daily_rows_30d": community_daily_rows_30d,
        "weekly_chart": build_total_chart(day_keys_7),
        "monthly_chart": build_total_chart(day_keys_30)
    }
    return set_no_store(make_json_response(payload))
